...
To run this recipe:
> python experiment_ami.py hparams/<your_hyperparams_file.yaml>
 e.g., python experiment_ami.py hparams/ecapa_tdnn.yaml
 ...
 * Jule Pohlhausen 2023: based on AMI-diarization-setup rttms
"""
...
def diarize_dataset(full_meta, split_type, n_lambdas, pval, n_neighbors=10):
    ...
    if params["oracle_n_spkrs"] is True:
        full_ref_rttm_file = (
            params["ref_rttm_dir"] + "/fullref_ami_" + split_type + "_" + params["annot_type"] + ".rttm"
        )
...
def dev_pval_tuner(full_meta, split_type, ref_rttm):
...
def dev_ahc_threshold_tuner(full_meta, split_type, ref_rttm):
...
def dev_nn_tuner(full_meta, split_type, ref_rttm):
...
def dev_tuner(full_meta, split_type, ref_rttm):
...
if __name__ == "__main__":  # noqa: C901
    ...
    from ami_setup_prepare import prepare_ami  # noqa
    ...
    dev_ref_rttm = os.path.join(params["ref_rttm_dir"], "fullref_ami_dev_" + params["annot_type"] + ".rttm")
    ...
    if params["affinity"] == "nn":
        ...
        best_nn = dev_nn_tuner(full_meta, "dev", dev_ref_rttm)
    ...
    if params["affinity"] == "cos" and (
        ...
        best_pval = dev_pval_tuner(full_meta, "dev", dev_ref_rttm)

    elif params["backend"] == "AHC":
        logger.info("Tuning for threshold-value for AHC")
        best_threshold = dev_ahc_threshold_tuner(full_meta, "dev", dev_ref_rttm)
        ...
        if params["oracle_n_spkrs"] is False:
            ...
            n_lambdas = dev_tuner(full_meta, "dev", dev_ref_rttm)
