...
Prepares metadata files (JSON) from AMI-diarization-setup [1] using RTTM format (Oracle VAD).
[1] F. Landini, J. Profant, M. Diez, L. Burget: [Bayesian HMM clustering of x-vector sequences (VBx) 
    in speaker diarization: theory, implementation and analysis on standard tasks]
...
def prepare_ami(
    ...
    ami_setup_folder : str
        Directory where the ami setup RTTMs are stored.
    save_folder : str
    ...
    annot_type : str
        Diarization setup for AMI corpus. See AMI-diarization-setup for more information.
        Based on Full-corpus-ASR partition, see split_type
        Allowed annot_type: "only_words" or "word_and_vocalsounds"
    split_type : str
        Standard dataset split. See ami_splits.py for more information.
        Allowed split_type: "full_corpus_asr" ("scenario_only", "full_corpus")
    skip_TNO: bool
    ...
    >>> data_folder = '/network/datasets/ami/amicorpus/AMI-diarization-setup'
    >>> save_folder = 'results/save/'
    >>> split_type = 'full_corpus_asr'
    >>> mic_type = 'Lapel'
    >>> prepare_ami(data_folder, ami_setup_folder, save_folder, split_type, mic_type)
    """
    ...
    for i in splits:
        rttm_file = ref_rttm_dir + "/fullref_ami_"+ i + "_" + annot_type + ".rttm"
        if i == "train":
    ...
    for i in splits:
        rttm_file = ref_rttm_dir + "/fullref_ami_"+ i + "_" + annot_type + ".rttm"
        meta_filename_prefix = "ami_" + i + "." + annot_type
        prepare_metadata(
            rttm_file,
            meta_data_dir,
            data_folder,
...
def prepare_segs_for_RTTM(
    list_ids, out_rttm_file, audio_dir, annot_dir, annot_type, split, skip_TNO
):
    ...
    for main_meet_id in list_ids:
        ...
        for sess in list_sessions:
            ...
            rttm_file = os.path.join(annot_dir, annot_type, "rttms", split, rec_id + ".rttm")
            ...
            with open(rttm_file, "r") as f:
                for line in f:
                    entry = line[:-1]
                    rttm_per_rec.append(entry)
                    spkrs_list.append(line.split(" ")[7])
