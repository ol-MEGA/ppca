...
def verification_performance(scores_plda, single_random_comparison=False):
    ...
    for line in open(veri_file_path):
        ...
        for idx,elem in enumerate(scores_plda.modelset):
            if elem.split("_")[0] == lab:
                    enroll_idxs.append(idx)
        if single_random_comparison:
            enroll_idxs = [random.choice(enroll_idxs)]
...
if __name__ == "__main__":
    ...
    # Final EER computation
    eer, min_dcf = verification_performance(scores_plda, single_random_comparison=False)
