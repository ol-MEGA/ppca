#!/usr/bin/env python3
"""Recipe for finetuning a Transformer ASR system with librispeech.
The system employs an encoder, a decoder, and an attention mechanism
between them. Decoding is performed with (CTC/Att joint) beamsearch coupled with a neural
language model.

To run this recipe, do the following:
> python finetune.py hparams/transformer.yaml
...
are used as basic recognition tokens. Finetuning is performed on the 360 h
...
class ASR(sb.core.Brain):
    def compute_forward(self, batch, stage):
        ...
        if hasattr(self.hparams.compute_features, "mel_equal_idx"):
            feats[:, :, self.hparams.compute_features.mel_equal_idx:] = -5
...
def dataio_prepare(hparams):
    ...
    valid_data = valid_data.filtered_sorted(sort_key="length", reverse=True)
    ...
    for csv_file in hparams["test_csv"]:
        ...
        if 'clean' in name:
            test_datasets[name] = sb.dataio.dataset.DynamicItemDataset.from_json(
                json_path=csv_file, replacements={"data_root": hparams["data_folder_clean"]}
            )
        else:
            test_datasets[name] = sb.dataio.dataset.DynamicItemDataset.from_json(
                json_path=csv_file, replacements={"data_root": hparams["data_folder_other"]}
            )
...
if __name__ == "__main__":
    ...
    for k in test_datasets.keys():  # keys are test_clean, test_other etc
        asr_brain.hparams.wer_file = os.path.join(
                hparams["output_folder"], "wer_{}.txt".format(k.replace('processed', hparams["proc_type"]))
            )
        asr_brain.evaluate(
            test_datasets[k],
            max_key="ACC",
